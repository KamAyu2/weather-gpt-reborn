...
import { MapPin, Maximize2, Minimize2, RefreshCw, Thermometer, X, ChevronUp, Layers } from "lucide-react";
...
function getTemperatureGlow(temp: number): string {
  const color = getTemperatureColor(temp);
  return `0 0 ${temp > 35 ? 20 : 12}px ${color}60`;
}
...
function MapView({ cityData, isMobile, onCitySelect }: {
    ...
    return (
      <div className="h-64 sm:h-80 bg-gradient-to-br from-blue-50 to-cyan-50 dark:from-blue-950/30 dark:to-cyan-950/30 flex items-center justify-center rounded-xl">
        <div className="text-center">
          <RefreshCw className="h-8 w-8 text-muted-foreground/30 mx-auto mb-2 animate-spin" />
  ...
  function MapEvents() {
    ...
    useEffect(() => {
      if (map) {
        // Disable scroll zoom on mobile by default, allow pinch
        if (isMobile) {
...
export function ThermalMap() {
  ...
  const fetchAllWeather = useCallback(async () => {
    ...
    try {
      ...
      setCityData(MAJOR_CITIES.map(city => ({
        name: city.name,
        lat: city.lat,
        lon: city.lon,
  ...
  const hottest = cityData.reduce((max, c) => c.temp > max.temp ? c : max, cityData[0]);
  const coldest = cityData.reduce((min, c) => c.temp < min.temp ? c : min, cityData[0]);
  ...
  return (
    <motion.div
      ...
      className={`rounded-2xl border border-border/50 bg-gradient-to-br from-white to-primary/5 shadow-sm overflow-hidden ${
        expanded && isMobile ? "fixed inset-0 z-50 rounded-none" : ""
      }`}
    >
      {/* Header */}
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-muted-foreground">
              {loading ? "Loading..." : `${cityData.length} cities • Live data`}
            </p>
          </div>
        </div>
          ...
          {hottest && coldest && !loading && (
            <div className="hidden sm:flex items-center gap-2 mr-2 text-[10px]">
              <span className="flex items-center gap-1 text-orange-600">
                🔥 {hottest.name} {Math.round(hottest.temp)}°C
              </span>
              <span className="text-muted-foreground/30">|</span>
              <span className="flex items-center gap-1 text-blue-600">
                ❄️ {coldest.name} {Math.round(coldest.temp)}°C
              </span>
            </div>
          )}
          <button
      ...
      <div className="relative">
        <MapView cityData={cityData} isMobile={isMobile} onCitySelect={setSelectedCity} />
      ...
      {/* City list panel (mobile-friendly) */}
      <AnimatePresence>
        {showCityList && (
          <motion.div
          ...
          >
            <div className="p-3">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">All Cities</span>
                <div className="flex gap-1">
                  <button
                    onClick={() => setSortBy("temp")}
                    className={`rounded-md px-2 py-0.5 text-[10px] ${sortBy === "temp" ? "bg-primary/10 text-primary" : "text-muted-foreground"}`}
                  >
                    By Temp
                  </button>
                  <button
                    onClick={() => setSortBy("name")}
                    className={`rounded-md px-2 py-0.5 text-[10px] ${sortBy === "name" ? "bg-primary/10 text-primary" : "text-muted-foreground"}`}
                  >
                    A-Z
                  </button>
                </div>
              ...
              <div className="max-h-48 overflow-y-auto space-y-1 pr-1">
                {sortedCities.map((city) => (
                  <button
                    ...
                    className="flex items-center justify-between w-full rounded-lg px-2.5 py-1.5 text-left hover:bg-muted/50 transition-colors"
                  >
                    <div className="flex items-center gap-2">
                      <div
                        className="w-2.5 h-2.5 rounded-full shrink-0"
                        style={{ backgroundColor: getTemperatureColor(city.temp) }}
                      />
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-[11px] font-semibold tabular-nums" style={{ color: getTemperatureColor(city.temp) }}>
                        {Math.round(city.temp)}°C
                      </span>
                    </div>
                  </button>
            ...
            <div className="p-4">
              <div className="flex items-start justify-between mb-3">
                <div>
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    {getWeatherEmoji(selectedCity.weatherCode)} {getWeatherDescription(selectedCity.weatherCode)}
                  </p>
                </div>
                <button
              ...
              <div className="flex items-baseline gap-2 mb-3">
                <span
                  ...
                  style={{ color: getTemperatureColor(selectedCity.temp) }}
                >
                  {Math.round(selectedCity.temp)}°C
                </span>
                <span className="text-xs text-muted-foreground">
                  Feels like {Math.round(selectedCity.feelsLike)}°C
                </span>
              </div>
              ...
              <div className="flex items-center gap-1 mb-3">
                <span
                  ...
                  style={{
                    backgroundColor: getTemperatureColor(selectedCity.temp) + "15",
                    color: getTemperatureColor(selectedCity.temp),
                  }}
                >
                  {getTempLabel(selectedCity.temp)}
                </span>
              </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-0.5">
            {[
              ...
              { label: "5-15°", color: "#0891b2" },
              { label: "15-25°", color: "#16a34a" },
              { label: "25-32°", color: "#ca8a04" },
              { label: "32-38°", color: "#ea580c" },
              { label: "38-42°", color: "#dc2626" },
              { label: ">42°", color: "#991b1b" },
            ].map((item) => (
